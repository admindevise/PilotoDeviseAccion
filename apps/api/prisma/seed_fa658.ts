...
async function main() {
  ...
  const jsonPath = path.resolve(__dirname, '../../../data/json_extraidos/auditoria_FA658_Hotel_B3_Virrey_COMPLETA.json');
  ...
  // Subcategoria mapping for Hotel B3 Virrey hallazgos
  const subcategoriaMap: Record<string, SubcategoriaHallazgo> = {
    'H-001': 'CARTERA_VENCIDA',      // Jul-ago 2025 facturas emitidas, impagas
    'H-002': 'CARTERA_VENCIDA',      // Cesión Peña Díaz facturada, impaga 21 meses
    'H-003': 'REVENUE_NO_CAPTURADO', // 16 pignoraciones nunca facturadas
  };
  ...
  for (const h of data.hallazgos) {
    ...
    if (h.severidad === 'CRITICO') sevE = 'CRITICO';
    
    await prisma.hallazgo.create({
      data: {
        fideicomisoId: fid.id,
        tipo: h.tipo ? h.tipo as TipoHallazgo : 'INCONSISTENCIA_FUENTES',
        severidad: sevE,
        categoria: h.categoria ? h.categoria as CategoriaHallazgo : 'CONSISTENCIA',
        subcategoria: subcategoriaMap[h.id] || 'ANOMALIA',
        area: h.area ? h.area as AreaHallazgo : 'OPERATIVA',
        titulo: h.titulo ? h.titulo.substring(0, 150) : 'Sin título',
        ...
        estado: h.estado ? h.estado as EstadoHallazgo : 'ABIERTO',
      }
    });
  }

  console.log('Seed de FA-658 (Hotel B3 Virrey) completado correctamente!');
}
